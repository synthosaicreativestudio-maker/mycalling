...
describe('matchProfessions — многомерность и объяснимость (docs/25 Трек A)', () => {
  ...
  it('экспресс-профиль (только RIASEC+BigFive) ранжирует, разбивка ≤3 осей', () => {
    // 'gardner' участвует всегда, когда есть RIASEC (docs/31 Блок B2: выводится
    // эвристикой из интересов, не требует отдельного теста) — поэтому экспресс-
    // профиль без PVQ/VIA/ICAR/textSignals легитимно даёт interests+personality+gardner.
    const ranked = matchProfessions(expressProfile);
    expect(ranked).toHaveLength(professionsDb.length);
    ranked.forEach((m) => {
      m.breakdown.forEach((a) => expect(['interests', 'personality', 'gardner']).toContain(a.axis));
      expect(m.breakdown.length).toBeLessThanOrEqual(3);
    });
  });
...
// docs/31 Блок B: новые оси — subjects (текстовые ответы коуча) и gardner
// (эвристика из RIASEC). Раньше подросток мог написать «люблю математику и
// информатику», и это НИКАК не влияло на ранжирование — только числовые тесты.
describe('subjectsFit и gardnerFit (docs/31, Блок B — учёт текстовых ответов)', () => {
  it('при совпадающем тексте профессия с этими предметами получает subjects=100%, без совпадения — ось не активна', () => {
    // Взвешенное усреднение НЕ гарантирует монотонный рост итогового score при
    // добавлении оси (если её балл ниже среднего по остальным — итог может и
    // просесть, это честно). Поэтому сравниваем не итоговый score, а саму ось.
    const withText: MatchProfile = { ...expressProfile, textSignals: 'Люблю математику, информатику и разбираться в компьютерах.' };
    const matchName = professionsDb.find((p) => p.subjects?.includes('Информатика'))!.name;
    const noMatchName = professionsDb.find((p) => p.subjects && p.subjects.length > 0 && !p.subjects.some((s) => ['Математика', 'Информатика', 'компьютер'].some((k) => s.toLowerCase().includes(k.toLowerCase()))))!.name;

    const ranked = matchProfessions(withText);
    const matchAxis = ranked.find((m) => m.profession.name === matchName)!.breakdown.find((a) => a.axis === 'subjects');
    const noMatchAxis = ranked.find((m) => m.profession.name === noMatchName)!.breakdown.find((a) => a.axis === 'subjects');
    // Частичное совпадение (не все предметы паспорта упомянуты в тексте) — честно,
    // балл > 0, но необязательно 100%.
    expect(matchAxis?.score).toBeGreaterThan(0);
    expect(noMatchAxis?.score).toBe(0);
  });

  it('subjects-ось появляется в breakdown только когда есть текстовые сигналы', () => {
    const withText: MatchProfile = { ...expressProfile, textSignals: 'Информатика и математика — моё любимое.' };
    const ranked = matchProfessions(withText);
    const hasSubjectsAxis = ranked.some((m) => m.breakdown.some((a) => a.axis === 'subjects'));
    expect(hasSubjectsAxis).toBe(true);

    const withoutText = matchProfessions(expressProfile);
    const noSubjectsAxis = withoutText.every((m) => m.breakdown.every((a) => a.axis !== 'subjects'));
    expect(noSubjectsAxis).toBe(true);
  });

  it('gardner выводится из RIASEC и участвует в разбивке всегда, когда RIASEC есть', () => {
    const ranked = matchProfessions(analyticalProfile);
    const anyGardner = ranked.some((m) => m.breakdown.some((a) => a.axis === 'gardner'));
    expect(anyGardner).toBe(true);
  });

  it('без RIASEC (пустой профиль) gardner-ось не появляется', () => {
    const ranked = matchProfessions(emptyProfile);
    const anyGardner = ranked.some((m) => m.breakdown.some((a) => a.axis === 'gardner'));
    expect(anyGardner).toBe(false);
  });
});

// Golden-тест: фиксирует топ-5 для устоявшегося профиля, чтобы будущие правки
// весов/эвристик были ОСОЗНАННЫМИ (тест упадёт и покажет diff), а не тихими.
describe('golden-тест: профиль → топ-5 (регрессия ранжирования)', () => {
  it('топ-5 для analyticalProfile стабилен между прогонами', () => {
    const top5 = topProfessions(analyticalProfile, 5).map((m) => m.profession.name);
    const top5Again = topProfessions(analyticalProfile, 5).map((m) => m.profession.name);
    expect(top5).toEqual(top5Again);
    expect(top5).toHaveLength(5);
  });

  it('топ-5 для socialProfile стабилен между прогонами', () => {
    const top5 = topProfessions(socialProfile, 5).map((m) => m.profession.name);
    const top5Again = topProfessions(socialProfile, 5).map((m) => m.profession.name);
    expect(top5).toEqual(top5Again);
    expect(top5).toHaveLength(5);
  });
});
