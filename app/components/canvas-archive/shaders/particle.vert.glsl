...
void main() {
    // Плавная интерполяция между начальной (position) и целевой точкой
    vec3 mixedPos = mix(position, aTargetPosition, uTransitionProgress);

    // Добавляем турбулентный Flow Field с помощью шума Simplex 3D
    ...
    // Смещение зависит от фазы (uFlowFieldStrength)
    ...
    // Расчет размера с учетом пикселей устройства и глубины сцены
    gl_PointSize = uSize * aScale * uPixelRatio * (300.0 / -mvPosition.z);

    // Цвета в эстетике Apple Vision Pro (переход от холодного голубовато-белого к насыщенному синему)
    ...
    // Передаем скорость (или силу морфинга) для постобработки
