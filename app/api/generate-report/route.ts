...
export async function POST(request: Request) {
  try {
    ...
    // 2. Отправляем запрос к ИИ claude-opus-4-7 через ProxyAPI
...
Ваша задача — проанализировать количественные результаты диагностики способностей, характера и интересов ученика и составить глубокий, вдохновляющий и научно обоснованный ИИ-отчет.
...
4. Профессии: Предложите от 3 до 5 современных, востребованных профессий (например: UX-дизайнер, промт-инженер, биоэтик, аналитик данных), обязательно объяснив в поле 'why', почему эта роль идеально подходит подростку на стыке его Хочу (интересы), Могу (интеллект) и Как (характер).
  ...
  "professions": [
    {
      ...
      "summary": "Краткое суть профессии.",
      "why": "Детальное ИИ-объяснение, как в этой профессии сходятся его интересы по RIASEC, черты характера по Big Five и природный интеллект по Гарднеру.",
...
Составь отчет по этим данным и верни строго JSON.`;
    ...
    console.error('Ошибка API-роута ИИ:', error);
