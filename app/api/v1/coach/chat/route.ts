...
const SCENARIO_STEPS = [
  ...
  {
    step: 1,
    goal: "Этап 1. Подключение канала связи. Предложить подростку подключить Telegram или MAX ID в качестве канала для отправки отчета. Объяснить, что это позволит сохранить результаты. Если пользователь предпочитает продолжить прямо здесь, он может написать свой номер телефона."
  },
  {
    step: 2,
    goal: "Этап 2. Имя и возраст. Спросить подростка, как его зовут (если имя еще не известно) и сколько ему лет. Начать мягкое знакомство."
  },
  {
    step: 3,
...
const FALLBACK_REPLIES: Record<number, string> = {
  ...
  1: "Слушай, а давай выберем удобный способ связи, чтобы я мог присылать тебе итоговый отчет и рекомендации? Проще всего подключить Telegram или MAX ID (результаты сразу сохранятся). Либо просто напиши свой номер телефона прямо здесь, если хочешь продолжить без мессенджера.",
  2: "Супер! Теперь давай познакомимся поближе. Расскажи, пожалуйста, как тебя зовут и сколько тебе лет?",
...
export async function POST(req: Request) {
  try {
    ...
    // и он находится на шаге 1, то автоматически перескакиваем на шаг 2 (знакомство)
    if (coachSession.user.phone && currentStep === 1) {
      currentStep = 2;
