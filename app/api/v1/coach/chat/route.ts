...
const SCENARIO_STEPS = [
  ...
  {
    step: 2,
    goal: "Этап 2. Подключение канала связи. Предложить подростку подключить Telegram или MAX ID в качестве канала для сохранения результатов и отправки отчета. Объяснить, что это позволит сохранить результаты."
  },
  {
    step: 3,
...
const FALLBACK_REPLIES: Record<number, string> = {
  ...
  3: "Очень приятно! Скажи, в каком городе ты живешь и в каком классе сейчас учишься?",
  4: "Интересно! А что для тебя ценнее всего в будущей работе — например, свобода творчества, хорошая зарплата, возможность помогать людям или что-то другое?",
...
${isRefusalOrEmpty ? '10. ВНИМАНИЕ: Пользователь ответил уклоничиво, прислал смайлик, многоточие или пустой мусорный ответ. Мягко обрати на это внимание с юмором или теплотой (например: "Кажется, ты прислал просто многоточие...") и дружелюбно попроси его все-таки поделиться мыслями по твоему предыдущему вопросу.' : ''}
`;
