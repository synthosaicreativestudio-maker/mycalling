...
import { scoreTalentSignals } from '../../../../lib/coach/talentSignals';
...
Извлекай: ${fieldsToExtract}`;
      ...
      {
        const signalScores = scoreTalentSignals(message);
        const signalScales = Object.keys(signalScores);
        if (signalScales.length > 0) {
          if (!extractedData.expressExtracted.talentScores) {
            extractedData.expressExtracted.talentScores = {
              creative: 0, tech: 0, science: 0, data: 0, social: 0, organizational: 0, startup: 0
            };
          ...
          for (const scale of signalScales) {
            scores[scale] = Math.max(scores[scale] || 0, (signalScores as Record<string, number>)[scale]);
          }
        }
      }
