...
graph TD
    ...
    CoachAPI -->|3. Инференс| ProxyAPI["ProxyAPI Claude Sonnet 4"]
...
Экстрактор работает на лету на каждом шаге диалога, анализируя сообщения пользователя через отдельный запрос к Claude Sonnet 4 (через ProxyAPI) с промтом-экстрактором:
